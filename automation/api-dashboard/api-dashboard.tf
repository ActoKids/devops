resource "aws_iam_role" "execution_role" {                            # Provides an IAM role - insert appropriate IAM role here
        name = "execution_role"
...
data "aws_iam_policy_document" "eventwatch_s3_json_full" {               # Creates json IAM policy
    statement {
        actions = [
            "s3:*",
        ]   
...
resource "aws_iam_policy" "eventwatch_s3_full" {                           # Provides an IAM policy
    ...
    path = "/"
    policy = "${data.aws_iam_policy_document.eventwatch_s3_json_full.json}"
}
...
resource "aws_lambda_function" "eventwatch_lambda" {                     # Attaches IAM Policy to an IAM role appropriately
...
resource "aws_lambda_permission" "allow_cloudwatch_ec2_events" {       # Creates a Lambda permission to allow external invoking of function
